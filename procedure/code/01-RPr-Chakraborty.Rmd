...
author: "Joseph Holler, Junyi Zhou, Peter Kedron, Drew An-Pham, Derrick Burt, Alana Lutz"
date: "`r Sys.Date()`"
...
I also want to add a version of the GEE model based on SatScan GINI-optimized clusters without incorporating the state ID clusters.
Comparing this GEE model to the rest will tell us how much the state clustering adds to the model.

### Unique GEE cluster IDs

First, calculate GEE cluster IDs.

We have already calculated: - `rp_clusID` based on our SpatialEpi clusters - `ss_clusID` based on our SaTScan cluster centers, and shown to be identical to the original author's data - `gini_clusID` based on our SaTScan GINI-optimized clusters - `gini_class` based on our SaTScan GINI-optimized clusters without incorporating State ID
...
### GINI Clusters WITHOUT State ID, and Fixed COVID-19 Rate

Calculate GEE models with: - Clustering: Reproduced SaTScan GEE clusters WITHOUT State ID - Dependent variable: reproduced COVID-19 incidence (fixed errors).

```{r gee-gini-clusters-nostate-reproduced-incidence}
gee_gini_nostate_rpdep <- onegee(gee_data,
                    dep_var = "covid_rate",
                    id = gee_data$gini_class,
                    term_names = table2$term)
gee_gini_nostate_rpdep$coefficients %>%
  kable(caption = "Reproduced SaTScan GINI Clusters WITHOUT State IDs and Reproduced COVID-19 Incidence",
        align = "c") %>%
  kable_styling()
```
...
geewhisker <- bind_rows(
  ...
  mutate(gee_gini_nostate_rpdep$coefficients,
         model = "SaTScan GINI-Optimized Clusters WITHOUT State ID")) %>%
  select(1:6, "model")
...
colors <- colorRampPalette(brewer.pal(4, "Spectral"))(6)
...
The figure above illustrates six model variations, starting with the results published in the original study.
...
Finally, removing the state ID clusters from the SaTScan GINI-optimized clusters slightly decreased the magnitude of most coefficients relative to the GEE model based on both GINI-optimized clusters and state IDs.
The greatest differences are seen in the Hispanic and Black models, in which the coefficients lost magnitude beyond the confidence intervals of the GINI-optimized cluster with state ID model.
This shift confirms that the Hispanic and Black coefficients are less robust than other coefficients to changes in analytical decisions.
Since these variables are unstable under different clustering criteria, they may have a weaker geographic relationship with COVID-19.
Despite changing the magnitude of certain coefficients, the new model (without state clustering) appears to have a similar amount of error to the other models.
...
This reproduction of Chakraborty's study tested the robustness of the original results to changes in computational environments and reanalysis with altered research parameters.
...
We also chose to create another model variation that removed the state clusters from the SaTScan GINI-optimized clusters.
The rationale for this reanalysis was that SaTScan is already accounting for potential spatial correlation both within and between states.
...
Further dividing the SaTScan clusters by state may introduce an assumption of correlation that doesn't exist, and state-level clustering is already accounted for by SaTScan spatial clustering.
We chose to specifically modify the SaTScan GINI-optimized cluster GEE because this cluster methodology seems to be the most robust.
However, it is worth noting that without incorporating state clustering, we produce a model with one very large cluster of no-risk areas.
Creating large clusters that are not divided by state may cause there to be very little correlation within clusters, resulting in counties with weights close to 1.
The fact that this model produced results similar to that of the other models suggests that geographic clustering plays only a small role in the overall analysis.
...
The broad conclusions of the original study and its contribution to scientific knowledge -- that PwDs are likely to experience multiple jeopardy for COVID-19 risk based on the intersection of their disability, racial/ethnic minority, and poverty status -- still hold true, despite uncertainty in the strength of certain individual relationships.
